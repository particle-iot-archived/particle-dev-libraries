...
export class LibraryCard extends View {
	...
	setLibrary(library) {
			...
			this.div(() => {
				this.h4({'class': 'card-name'}, () => {
					...
					if (library.visibility === 'private') {
						this.i({'class': 'icon icon-lock library-private', title: 'Private library'});
					}
					if (library.verified) {
						this.span({
							'class': 'library-verified',
							title: 'This library has been verified by Particle'
						});
					}
					if (library.official) {
						this.img({
							'src': 'atom://particle-dev/images/logo.png',
							'class': 'library-official',
		...
		if (!(core().isProject() || core().isLibrary())) {
