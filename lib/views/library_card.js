...
export class LibraryCard extends View {
	...
	setLibrary(library) {
		...
		this.find('#view-sources').on('click', () => {
			...
			const site = new DevLibraryInstallSite(core().getProjectDir(), (lib, targetDir) => {
			...
			site.setLibraryName(library.name);
		...
		this.find('#add-to-project').on('click', () => {
			...
			site.setLibraryIdent(library.name, library.version);
			runCommand(site.run(command)).then(() => {
				showLibraryAdded(library);
			});
		});
	}
}
