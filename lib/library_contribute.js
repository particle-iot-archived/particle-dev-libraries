...
export function libraryContributeCommand(atom, directory, dryRun=false) {
	...
	if (!directory) {
		...
		return runCommand(new DevLibraryContributeCommandSite(atom, apiClient, directory, dryRun).run(contribute, {})).catch((err) => {
			atom.notifications.addNotification(notifyContributionError(err));
		});
	}
}
