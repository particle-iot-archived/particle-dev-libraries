...
export function notifyLibraryPublishing(lib) {
    return new Notification('info', `Publishing library ${lib}...`);
}

export function notifyLibraryPublished(lib) {
    return new Notification('info', `Library ${lib} was successfully published! It's available for others now.`, {
        dismissable:true
    });
}
...
export function libraryPublishCommand(atom, directory) {
    ...
    if (!directory) {
        ...
        return runCommand(new DevLibraryPublishCommandSite(atom, apiClient, directory).run(publish, {})).catch(err => {
            atom.notifications.addNotification(publishingLibraryErrorNotification());
        });
    }
}
