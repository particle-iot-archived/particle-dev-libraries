...
class DevProjectInitView extends View {
	...
	initialize() {
		...
		this._directory = this.projects.myProjectsFolder();
		this._name = 'NewProject';
	...
	proceed() {
		...
		const promise = this.site.run(this.command);
		...
		this.showStep(2);
		return this.projects.ensureDirectoryExists(this.projects.myProjectsFolder())
			.then(() => promise)
	...
	openProject() {
		if (this.projectDirectory) {
			this.openNewWindow(this.projectDirectory);
		} else {
